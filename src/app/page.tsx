...
import {
    ...
    faCopy, faCheck, faChevronDown, faSpinner,
} from '@fortawesome/free-solid-svg-icons';

// Lazy-load heavy markdown renderer
const ReactMarkdown = dynamic(() => import('react-markdown'), { ssr: false, loading: () => <span className="text-[#A8A29E] text-sm">…</span> });
...
export default function Chat() {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (storedId && storedName) { setUserId(storedId); setUserName(storedName); setShowRegistration(false); }
        }
    }, []);

    // ─── Chat State ───────────────────────────────────────────
    const { messages, input, handleInputChange, handleSubmit, isLoading, append } = useChat({ body: { userId } });
    ...
    // Focus input
    useEffect(() => { if (!isLoading && !showRegistration) inputRef.current?.focus(); }, [isLoading, showRegistration]);
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        if (!userName.trim() || !userPhone.trim() || !userEmail.trim()) { setRegError('All fields are required.'); return; }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(userEmail.trim())) { setRegError('Please enter a valid email.'); return; }
        if (!/\d{10,}/.test(userPhone.replace(/[\s\-\+\(\)]/g, ''))) { setRegError('Please enter a valid phone number.'); return; }
        setRegLoading(true);
        try {
            const res = await fetch('/api/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: userName.trim(), phone: userPhone.trim(), email: userEmail.trim() }) });
            const data = await res.json();
            if (data.error) setRegError(data.error);
            else { setUserId(data.user.id); sessionStorage.setItem('dexterUserId', data.user.id); sessionStorage.setItem('dexterUserName', data.user.name); setShowRegistration(false); }
        } catch { setRegError('Network error. Please try again.'); }
        setRegLoading(false);
    };

    const handleSuggestionClick = (question: string) => { append({ role: 'user', content: question }); };
                    ...
                    <form onSubmit={handleRegister} className="space-y-4">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="e.g. Rahul Sharma" className="skeuo-input w-full p-3 sm:p-3.5 !pl-11 text-sm" autoFocus />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="tel" value={userPhone} onChange={(e) => setUserPhone(e.target.value)} placeholder="e.g. +91 98765 43210" className="skeuo-input w-full p-3 sm:p-3.5 !pl-11 text-sm" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="email" value={userEmail} onChange={(e) => setUserEmail(e.target.value)} placeholder="e.g. rahul@company.com" className="skeuo-input w-full p-3 sm:p-3.5 !pl-11 text-sm" />
                            </div>
                        </div>
                        {regError && (
                        ...
                        <button type="submit" disabled={regLoading} className="skeuo-brass w-full py-3 text-sm mt-2 flex items-center justify-center gap-2">
                            {regLoading ? <><FontAwesomeIcon icon={faSpinner} className="w-3.5 h-3.5 animate-spin" /> Registering...</> : 'Start Chat →'}
                        </button>
                    </form>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] sm:text-[11px] text-[#78716C] font-medium">
                                Hi, {userName} · বাংলায় প্রশ্ন করুন
                            </p>
                        </div>
                    </div>
                ...
                <div className="max-w-3xl mx-auto space-y-4 sm:space-y-5">
                    {messages.length === 0 ? (
                        <div className="text-center mt-8 sm:mt-12 lg:mt-16 animate-fade-up">
                            <div className="skeuo-card p-6 sm:p-8 lg:p-12">
                                ...
                                <h2 className="text-xl sm:text-2xl lg:text-3xl font-semibold mb-2 sm:mb-3 text-[#1C1917]">
                                    HMS প্যানেল সম্পর্কে জিজ্ঞাসা করুন
                                </h2>
                                <p className="text-[#78716C] max-w-md mx-auto leading-relaxed text-xs sm:text-sm">
                                    আমি আপনার Dexter HMS টেকনিক্যাল সাপোর্ট অ্যাসিস্ট্যান্ট।
                                    যেকোনো ইন্সটলেশন, ট্রাবলশুটিং বা নেটওয়ার্ক সমস্যায় সাহায্য করতে পারি।
                                </p>

                                <div className="mt-6 sm:mt-8 grid gap-2.5 sm:gap-3 sm:grid-cols-2 text-sm text-left">
                                    {suggestedQuestions.map((question, i) => (
                                        <button key={`${i}-${question.slice(0, 20)}`} onClick={() => handleSuggestionClick(question)}
                                            className="skeuo-raised group p-3 sm:p-4 text-[#44403C] text-left flex items-start gap-2.5 text-xs sm:text-sm">
                                            <FontAwesomeIcon icon={faArrowRight} className="w-2.5 h-2.5 sm:w-3 sm:h-3 mt-0.5 text-[#A8A29E] group-hover:text-[#CA8A04] transition-colors flex-shrink-0" />
                                ...
                                <div className="mt-5 sm:mt-6 flex flex-wrap justify-center gap-1.5 sm:gap-2">
                                    {['Modbus RTU', 'PROFIBUS', 'I/O fault', 'Power supply', 'Commissioning', 'Firmware update'].map((tag) => (
                        ...
                        messages.map((m) => (
                            <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-up`} style={{ contentVisibility: 'auto' }}>
                                <div className={`max-w-[92%] sm:max-w-[80%] rounded-2xl p-3.5 sm:p-4 lg:p-5 ${m.role === 'user'
                                    ? 'skeuo-leather rounded-tr-sm'
                                    : 'skeuo-card text-[#1C1917] rounded-tl-sm'
                                    }`}>
                                    {m.role !== 'user' && (
                                        <div className="flex items-center justify-between mb-2.5">
                                            ...
                                            {/* Copy button */}
                                            <button onClick={() => handleCopy(m.content, m.id)} className="p-1 rounded text-[#A8A29E] hover:text-[#CA8A04] transition-colors cursor-pointer" title="Copy response">
                                                <FontAwesomeIcon icon={copiedId === m.id ? faCheck : faCopy} className={`w-3 h-3 ${copiedId === m.id ? 'text-emerald-600' : ''}`} />
                                            </button>
                                        </div>
                                    )}
                                    <div className="text-sm sm:text-[15px] leading-relaxed">
                                        <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                                            ...
                                            strong: ({ node, ...props }: any) => <strong className={`font-semibold ${m.role === 'user' ? 'text-[#CA8A04]' : 'text-[#1C1917]'}`} {...props} />,
                                            ...
                                            code: ({ node, ...props }: any) => (
                                                <code className={`px-1.5 py-0.5 rounded text-xs sm:text-sm ${m.role === 'user'
                                                    ? 'bg-white/15 text-[#EAB308]'
                                                    : 'bg-[#F0EBE3] text-[#0D9488] border border-[#D6CFC4]'
                                                    }`} {...props} />
                                            ),
                                            img: ({ node, ...props }: any) => <img className="rounded-xl border border-[#D6CFC4] my-3 max-w-full shadow-md" loading="lazy" {...props} />,
                                        }}>
                                            {m.content}
                                        </ReactMarkdown>
                                    </div>
                                    {/* Footer: time badge + timestamp */}
                                    <div className="mt-2 flex items-center gap-3 text-[10px] text-[#A8A29E]">
                                        {m.role === 'assistant' && responseTimes.has(m.id) && (
                    ...
                    {isLoading && messages[messages.length - 1]?.role === 'user' && (
                        <div className="flex justify-start animate-fade-up">
                            <div className="skeuo-card rounded-2xl rounded-tl-sm px-4 py-3.5 sm:px-5 sm:py-4 max-w-[75%] sm:max-w-[70%] space-y-2.5">
                                ...
                                <span className="text-[10px] sm:text-[11px] text-[#A8A29E] block">বিশ্লেষণ ও উত্তর তৈরি হচ্ছে…</span>
                            </div>
                        </div>
                    )}
                ...
                <div className="max-w-3xl mx-auto">
                    <form onSubmit={handleSubmit} className="relative flex items-center">
                        <input ref={inputRef}
                            ...
                            value={input} placeholder="বাংলায় প্রশ্ন করুন… (যেমন: Modbus এরর কেন হচ্ছে?)"
                    ...
                    <p className="text-center text-[10px] sm:text-[11px] text-[#A8A29E] mt-2">
                        HMS প্যানেল বিশেষজ্ঞ · <span className="hidden sm:inline">Ctrl+Enter to send · </span>AI দ্বারা চালিত
                    </p>
                </div>
            </div>
        </div>
